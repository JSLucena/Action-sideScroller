...
func _enter_state(new_state, old_state):
	match new_state:
		states.idle:
			count = 0
			if parent.side == "right":
				parent.get_node("Character").get_node("idle").show()
				parent.get_node("Character").get_node("animations").play("Idle_R")
			else:
				parent.get_node("Character").get_node("idle_rev").show()
...
func _exit_state(old_state, new_state):
	match old_state:
		states.idle:
			parent.get_node("Character").get_node("idle_rev").hide()
			parent.get_node("Character").get_node("idle").hide()
		states.run:
			parent.get_node("Character").get_node("walk").hide()
			parent.get_node("Character").get_node("walk_rev").hide()
		states.jump:
